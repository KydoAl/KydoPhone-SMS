...
object SmsNotifications {
    ...
    private fun notificationId(address: String): Int = ("sms:" + PhoneUtils.matchKey(address)).hashCode()
    ...
    fun showIncoming(context: Context, address: String, body: String, whenMillis: Long) {
        ...
        val name = try {
            ...
            ContactsRepository(c).loadAll().firstOrNull { c2 -> c2.numbers.any { PhoneUtils.matchKey(it.number) == key } }?.name
        } catch (e: Exception) {
            null
        }
        ...
        val builder = Notification.Builder(context, CH_MESSAGES)
